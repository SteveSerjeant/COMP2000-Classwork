    ...
    public ViewEditor()  {
        ...
        walkSpn.addChangeListener(
                new ChangeListener() {
                    @Override
                    public void stateChanged(ChangeEvent e) {
                        controller.setModelProperty(new KeyValuePair(AbstractController.WALK_SPEED, doubleToFloat((double)walkSpn.getValue())));
                    }
                }
        );

        runSpn = new JSpinner(new SpinnerNumberModel(6.0, 6.0, 12.0, 0.1));
        runSpn.addChangeListener(
                new ChangeListener() {
                    @Override
                    public void stateChanged(ChangeEvent e) {
                        double value = (double) runSpn.getValue();
                        controller.setModelProperty(new KeyValuePair(AbstractController.RUN_SPEED,doubleToFloat((double)runSpn.getValue())));
                    }
                }
        );
        ...
        jumpSpn.addChangeListener(
                new ChangeListener() {
                    ...
                    public void stateChanged(ChangeEvent e) {
                        ...
                        controller.setModelProperty(new KeyValuePair(AbstractController.JUMP_HEIGHT,doubleToFloat((double)jumpSpn.getValue())));
                    }
                }
        );
